 ...
 *
 */
public class NoticeAnnouncementActvity extends BaseDynamiclistviewActivity implements OnItemClickListener,RefreshListener,LoadMoreListener,OnVolleyResponseListener{
	...
	public void initData() {
		...
		getVolleyUtil().setJSONObject(INIT_WHAT,HttpConfig.NOTICE_LIST_URL + HttpConfig.CALLBACK_JSON);
	}
	...
	public boolean onRefresh(DynamicListView dynamicListView) {
		getVolleyUtil().setJSONObject(REFRESH_WHAT, HttpConfig.NOTICE_LIST_URL + HttpConfig.CALLBACK_JSON);
		return false;
	}
	...
	public boolean onLoadMore(DynamicListView dynamicListView) {
		if (adapter.getCount() % JsonDataConfig.LIST_NUM == 0) {
			getVolleyUtil().setJSONObject(LOADMORE_WHAT, HttpConfig.NOTICE_LIST_URL + HttpConfig.CALLBACK_JSON + HttpConfig.LIST_NEXT_URL + (adapter.getCount() / JsonDataConfig.LIST_NUM + 1));
		}else {
